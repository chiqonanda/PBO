...
public class AplikasiService {
    ...
    if (daftarApp.isEmpty()) {
        ...
        for (int i = 0; i < daftarApp.size(); i++) {
            Aplikasi app = daftarApp.get(i);
            System.out.println((i+1) + ". " + app.infoSingkat()); // Polymorphism
...
public void updateAplikasi() {
    ...
    if (!daftarApp.isEmpty()) {
        ...
        if (idx >= 0 && idx < daftarApp.size()) {
            Aplikasi appLama = daftarApp.get(idx);

            // Input baru (atribut umum)
            ...
            // Cek tipe object lama → buat object baru dengan tipe yang sama
            if (appLama instanceof Game) {
                String genre = inputString("Genre Game baru: ");
                daftarApp.set(idx, new Game(nama, dev, versi, rating, download, kategori, deskripsi, ukuran, genre));

            } else if (appLama instanceof SosialMedia) {
                int pengguna = (int) inputLong("Jumlah Pengguna Aktif baru: ");
                daftarApp.set(idx, new SosialMedia(nama, dev, versi, rating, download, kategori, deskripsi, ukuran, pengguna));

            } else if (appLama instanceof Edukasi) {
                String tingkat = inputString("Tingkat Pendidikan baru: ");
                daftarApp.set(idx, new Edukasi(nama, dev, versi, rating, download, kategori, deskripsi, ukuran, tingkat));

            } else if (appLama instanceof AplikasiPembantu) {
                boolean butuhNet = inputString("Butuh Internet? (ya/tidak): ").equalsIgnoreCase("ya");
                daftarApp.set(idx, new AplikasiPembantu(nama, dev, versi, rating, download, kategori, deskripsi, ukuran, butuhNet));
            }
    ...
    // Overloading: versi langsung tanpa input scanner
    public void tambahAplikasi(Aplikasi app) {
    daftarApp.add(app);
    System.out.println("Aplikasi berhasil ditambahkan (via overloading)!");
